...
) noexcept
{
  ...
  const auto beta = (3.0 * n / 2.0) + (-27.0 * pow_int<3>(n) / 32.0)
                  + (269.0 * pow_int<5>(n) / 512.0);
...
) noexcept
{
  ...
  /* Calculate easting (x) */
  *x = n * c * l + (n / 6.0 * pow_int<3>(c) * l3_coefficient * pow_int<3>(l))
     + (n / 120.0 * pow_int<5>(c) * l5_coefficient * pow_int<5>(l))
     + (n / 5040.0 * pow_int<7>(c) * l7_coefficient * pow_int<7>(l));
  /* Calculate northing (y) */
  *y = arc_length_of_meridian(phi) + (t / 2.0 * n * pow_int<2>(c) * pow_int<2>(l))
...
) noexcept
{
  ...
  /* Calculate latitude */
  *phi = phi_f + x2_fraction * x2_polynomial * (x * x) + x4_fraction * x4_polynomial * pow_int<4>(x)
       + x6_fraction * x6_polynomial * pow_int<6>(x) + x8_fraction * x8_polynomial * pow_int<8>(x);
  /* Calculate longitude */
  *lambda = lambda0 + x1_fraction * x + x3_fraction * x3_polynomial * pow_int<3>(x)

...
public:
  ...
  MergeIterator(
    ...
    : MergeIterator(lhs.begin(), lhs.end(), rhs.begin(), rhs.end())
  {
  }
  const value_type&
  operator*() const
  ...
  auto
  operator<=>(const MergeIterator&) const = default;   // three-way comparison C++20
  auto
  begin()
  {
    return start_;
...
)
{
  ...
  {
    auto m1 = MergeIterator<merged_map_type, merged_map_type>(r, m0);
    auto it_m = m1.begin();
    while (m1.end() != it_m)

...
FireWeather::FireWeather(
  ...
  : FireWeather(used_fuels, min_date, max_date, [&]() {
      auto it = std::views::transform(weather_by_hour_by_day, [](auto& v) {
        return make_wx(v);
      });
...
static vector<ptr<const FwiWeather>>
...
ThresholdSize
FireWeather::survivalProbability(
  const DurationSize time,

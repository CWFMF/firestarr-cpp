...
) noexcept
{
  ...
  const auto cos_h = (_cos(Zenith) - sin_dec * _sin(util::to_radians(latitude)))
                   / (cos_dec * _cos(util::to_radians(latitude)));
  if (cos_h > 1)
  {
    // sun never rises
...
) noexcept
{
  ...
  {
    days[i] = make_tuple(
      fix_hours(sunrise(static_cast<int>(i), latitude, longitude) + sim::Settings::offsetSunrise()),
      fix_hours(sunset(static_cast<int>(i), latitude, longitude) - sim::Settings::offsetSunset())
    );

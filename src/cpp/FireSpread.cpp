...
)
{
  ...
  const auto key = Cell::key(
    Cell::hashCell(slope, aspect, FuelType::safeCode(lookup.byName(fuel_name)))
  );
  ...
  logging::check_equal(
    a,
    (static_cast<SlopeSize>(0) == slope ? static_cast<AspectSize>(0) : aspect),
    "aspect"
  );
    ...
    nd_(nd)
{
  ...
  const auto no_correction = [](const MathSize) noexcept {
    return 1.0;
  };
  ...
  const auto add_offsets = [&correction_factor,
                            &add_offset,
                            raz,
                            min_ros](const MathSize angle_radians, const MathSize ros_flat) {
    if (ros_flat < min_ros)
    {
      return false;
    }

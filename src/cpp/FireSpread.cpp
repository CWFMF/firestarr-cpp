...
)
{
  ...
  else
  {
    ...
    spread.is_crown_ = fuel::FuelType::isCrown(critical_surface_intensity, sfi);
    if (spread.is_crown_)
    {
...
)
{
  ...
  const auto key = topo::Cell::key(
    topo::Cell::hashCell(slope, aspect, fuel::FuelType::safeCode(lookup.byName(fuel_name)))
  );
  ...
  logging::check_fatal(
    a != aspect_expected,
    "Expected aspect to be %d but got %d",
    aspect_expected,
    a
  );
  logging::check_fatal(
    ...
    nd_(nd)
{
  ...
  const auto add_offsets = [&correction_factor,
                            &add_offset,
                            raz,
                            min_ros](const double angle_radians, const double ros_flat) {
    if (ros_flat < min_ros)
    {
      return false;
    }
...
#define STEP_MAX util::to_radians(STEP_MAX_DEGREES)
  ...
  while (added && cur_x < 1.0)
  {
    ...
    // added = add_offsets_calc_ros(theta);
    printf(
      "cur_x = %f, theta = %f, angle = %f, last_theta = %f, last_angle = %f\n",
      cur_x,
  ...
  last_theta = util::to_radians(
    static_cast<int>(util::to_degrees(last_theta) / STEP_MAX_DEGREES) * STEP_MAX_DEGREES
  );

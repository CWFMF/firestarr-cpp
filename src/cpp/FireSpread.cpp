    ...
    nd_(nd)
{
  ...
  if (!has_no_slope)
  {
    ...
    {
      wse = 28.0
          - log(1.0 - min(0.999 * 2.496 * ffmc_effect, isf1) / (2.496 * ffmc_effect)) / 0.0818;
    }
  ...
  head_ros_ = fuel->calculateRos(nd, *weather, isi) * bui_eff;
  ...
  const auto is_crown = fuel::FuelType::isCrown(
  ...
  if (is_crown)
  {
    head_ros_ = fuel->finalRos(*this, isi, fuel->crownFractionBurned(head_ros_, rso), head_ros_);
  ...
  if (head_ros_ < threshold || sfc < COMPARE_LIMIT
  ...
  const auto add_offsets = [&correction_factor,
                            &add_offset,
                            raz,
                            threshold](const double angle_radians, const double ros_flat) {
  ...
  if (add_offsets_calc_ros(util::to_radians(10)) && add_offsets_calc_ros(util::to_radians(20))
      && add_offsets_calc_ros(util::to_radians(30)) && add_offsets_calc_ros(util::to_radians(60))
      && add_offsets(util::to_radians(90.0), flank_ros * sqrt(a_sq_sub_c_sq) / a)
      && add_offsets_calc_ros(util::to_radians(120)) && add_offsets_calc_ros(util::to_radians(150)))
  {

...
)
{
  ...
  auto points_and_sources = std::views::transform(
    kv0.second,
    [&duration, &offsets](const tuple<Cell, PointSet> pts_for_cell) {
      return merge_list(duration, std::get<0>(pts_for_cell), std::get<1>(pts_for_cell), offsets);
    }
  );

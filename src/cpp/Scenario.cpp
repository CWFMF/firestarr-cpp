...
)
{
  ...
  auto can_spread = [this, &spreading, &mutex_spreading, &new_time](const topo::Cell for_cell) {
  ...
  auto apply_offsets = [this, &new_time, &duration, &sources, &can_spread](
    ...
    for (auto& kv : points_map)
    {
      ...
      if (!(*unburnable_)[for_cell.hash()])
      {
        ...
        // // // since we only generated spread_info_ at the start it only exists for cells from end
        // of last step
        // // const auto& origin_inserted = spread_info_.try_emplace(key, *this, time, key,
        // nd(time), wx);
        // // // // any cell that has the same fuel, slope, and aspect has the same spread
        // // const auto& origin = origin_inserted.first->second;
        // // // const auto& seek_spread = spread_info_.find(key);
        // // // const auto max_intensity = (spread_info_.end() == seek_spread) ? 0 :
  ...
  do_each(points_cur, [this, &sources, &new_time](pair<const topo::Cell, PointSet>& kv) {

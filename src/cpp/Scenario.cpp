...
)
{
  ...
  // std::transform(points_.cbegin(), points_.cend(), keys.begin(), [](const pair<const topo::Cell,
  // const PointSet>& kv) { return kv.first.key(); });
  auto keys = std::set<topo::SpreadKey>();
  std::transform(
    points_.cbegin(),
    points_.cend(),
    std::inserter(keys, keys.begin()),
    [](const pair<const topo::Cell, const PointSet>& kv) {
      return kv.first.key();
    }
  );
  ...
  for (auto& location : std::vector<topo::Cell>(cells_old.begin(), cells_old.end()))
  {
    ...
    else
    {
      ...
      logging::check_fatal(offsets.empty(), "Shouldn't add empty offsets");
      logging::check_fatal(
        std::get<2>(to_spread.at(to_spread.size() - 1))->empty(),
  ...
  for (auto& t : to_spread)
  {
    ...
    // const auto key = location.key();
    // auto& offsets = spread_info_.at(key).offsets();
    logging::check_fatal(offsets.empty(), "Shouldn't have any empty offsets in this loop");
    for (auto& o : offsets)
    {
      // offsets in meters

...
)
{
  ...
  switch (event.type())
  {
    ...
    case Event::NEW_FIRE:
      ...
      points_new_.insert((*intensity_new_->unburnable_)[p.hash()], p0);
...
)
{
  ...
  else
  {
    ...
    for (const auto& location : perimeter_->edge)
    {
      ...
      points_new_.insert((*intensity_new_->unburnable_)[p0.hash()], p0);
    ...
    for (const auto& pt : pts)
    {
      for (const ROSOffset& r_p : offsets_after_duration)
      {
        ...
        const auto& y_o = out.y();
        {
          ...
          points_new.insert((*scenario.intensity_new_->unburnable_)[p0.hash()], p0);
        }
      }
    }
  }
...
)
{
  ...
  for (auto& kv0 : to_spread_new)
  {
    ...
    // auto r =
    apply_offsets_spreadkey(cell_pts_new, pts_spreading_new, *this, new_time, duration, offsets);
    // for (XYPos& p : r)
    // {
  ...
  while (it != points_new_.map_.end())
  {
    ...
    if (intensity_new_->hasNotBurned(hash_value)

...
public:
      ...
      )}
  {
    ...
    const auto offset_x = static_cast<Idx>(
      (this->xllcorner() - perim_grid.xllcorner()) / this->cellSize()
    );
    const auto perim_origin = static_cast<Idx>(
      perim_grid.rows() + perim_grid.yllcorner() / this->cellSize()
    );
...
{
  const auto perim_grid = ConstantGrid<unsigned char>::readTiff(perim, point);
  ...
  for (auto c : edge_)
  {
    logging::check_fatal(is_null_fuel(env.cell(c)), "Null fuel in perimeter edge");
  }
#endif
}
    ...
    for (auto x = -range; x <= range && num_cells > count; ++x)
    {
      for (auto y = -range; y <= range && num_cells > count; ++y)
      {
        ...
        if (sqrt(pow_int<2>(x) + pow_int<2>(y)) < max_distance)
        {
          ...
          if (1 != perim_grid.at(hash_value) && !is_null_fuel(loc))
          {
            perim_grid.set(hash_value, 1);
            ++count;
          }

...
)
{
  ...
  const Startup* const use_startup = (!started || nullptr == startup) ? &fake_startup : startup;
  ...
  for (size_t date_index = 0; date_index < cur_member.size(); ++date_index)
  {
    ...
    if (date_index >= 2)
    {
      ...
      else if (((!shutdown && month > 9) || (9 == month && 15 <= for_date.day)) && date_index >= 2)
      {
    ...
    wx_.emplace_back(wx.tmp(), wx.rh(), wx.wind(), apcp, ffmc, dmc, dc, isi, bui, fwi);
    last_ffmc = ffmc;
    last_dmc = dmc;
    last_dc = dc;
    if (started && !shutdown)

...
) noexcept
{
  ...
  if (mo > ed)
  {
    ...
    const auto ko = 0.424 * (1.0 - pow(rh.value / 100.0, 1.7))
                  + 0.0694 * sqrt(ws.value) * (1.0 - pow_int<8>(rh.value / 100.0));
    //'''/* 6b '*/
  ...
  if (mo < ew)
  {
    ...
    const auto kl = 0.424 * (1.0 - pow((100.0 - rh.value) / 100.0, 1.7))
                  + 0.0694 * sqrt(ws.value) * (1 - pow_int<8>((100.0 - rh.value) / 100.0));
    //'''/* 7b '*/
...
//    prec is the 24-hour accumulated rainfall in mm, calculated at 12:00 LST
...
) noexcept
{
  ...
  const auto m = find_m(temperature, rh, ws, mo);
  //'''/* 10 '*/
...
Ffmc::Ffmc(
  ...
  : Ffmc(calculate_ffmc(temperature, rh, ws, prec, ffmc_previous))
{ }
//******************************************************************************************
// Function Name: DMC
...
) noexcept
{
  ...
  {
    //'''/* 11  '*/
    const auto re = 0.92 * prec.value - 1.27;
    //'''/* 12  '*/
...
Dmc::Dmc(
  ...
  : Dmc{calculate_dmc(temperature, rh, prec, dmc_previous, month, latitude)}
{ }
//******************************************************************************************
// Function Name: DC
...
) noexcept
{
  ...
  {
    //'/* 18  */
    const auto rd = 0.83 * (prec.value) - 1.27;
    //'/* 19  */
...
Dc::Dc(
  ...
  : Dc{calculate_dc(temperature, prec, dc_previous, month, latitude)}
{ }
...
//******************************************************************************************
Isi calculate_isi(const Speed& ws, const Ffmc& ffmc) noexcept
{
  //'''/* 24  '*/
  const auto f_wind = exp(0.05039 * ws.value);
  const auto f_f = ffmc_effect(ffmc);
  //'''/* 26  '*/
  return {(0.208 * f_wind * f_f)};
}
Isi::Isi(const Speed& ws, const Ffmc& ffmc) noexcept : Isi{calculate_isi(ws, ffmc)} { }
Isi check_isi(
  const MathSize
...
//******************************************************************************************
Bui calculate_bui(const Dmc& dmc, const Dc& dc) noexcept
{
...
Bui::Bui(const Dmc& dmc, const Dc& dc) noexcept : Bui{calculate_bui(dmc, dc)} { }
Bui check_bui(
  MathSize
#if defined(CHECK_CALCULATION) | defined(USE_GIVEN)
    value
...
//******************************************************************************************
Fwi calculate_fwi(const Isi& isi, const Bui& bui) noexcept
{
...
Fwi::Fwi(const Isi& isi, const Bui& bui) noexcept : Fwi{calculate_fwi(isi, bui)} { }
Fwi check_fwi(
  MathSize
#if defined(CHECK_CALCULATION) | defined(USE_GIVEN)
    value
...
//******************************************************************************************
Dsr calculate_dsr(const Fwi& fwi) noexcept
{
  //'''/* 41 '*/

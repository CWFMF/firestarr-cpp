...
public:
  ...
  FireWeather&
  operator=(FireWeather&& rhs) noexcept = default;
  FireWeather&
  operator=(const FireWeather& rhs) = delete;
  ...
  at(const DurationSize time) const;
  ...
  survivalProbability(const DurationSize time, const FuelCodeSize& in_fuel) const;

  /**
   * \brief Minimum date present in FireWeather
   * \return Minimum date present in FireWeather
   */
  [[nodiscard]] constexpr Day
  minDate() const
  {
    return min_date_;
  }
   ...
   */
  [[nodiscard]] const vector<ptr<const FwiWeather>>&
  getWeather()
  {

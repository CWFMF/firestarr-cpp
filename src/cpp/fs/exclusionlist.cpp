 ...
 */
void addFrame(excList* l) { l->top = newFrame(l->top); }
/*
...
excList* newExcList()
{
  auto* ret = (excList*)malloc(sizeof(excList));
  point nullPt = {-1, -1};
...
Frame* newFrame(Frame* next)
{
  ...
  Frame* ret;
  if (junkFrames)
  {   // recycle the Frame == (junkFrames != nullptr)
    ret = junkFrames;
    junkFrames = ret->next;
  }
  else
...
Node* newNode(point p, Node* fwd)
{
  ...
  Node* ret;
  if (junkNodes)
  {   // recycle the stackNode == (junkNodes != nullptr)
    ret = junkNodes;
    junkNodes = ret->fwd;
  }
  else
...
void popFrame(excList* l)
{
  Frame* tmpFrame;
  Node* curNode;
  while (l->top->first != nullptr)
  {   //'while (cur) {' same as 'while (cur != nullptr) {'
    curNode = l->top->first;

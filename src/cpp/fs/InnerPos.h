...
public:
   ...
   */
  inline constexpr InnerSize x() const noexcept { return x_; }
  /**
   * \brief Y direction (row)
   */
  inline constexpr InnerSize y() const noexcept { return y_; }
  constexpr BoundedPoint(const InnerSize x, const InnerSize y) noexcept : x_(x), y_(y)
  {
#ifdef DEBUG_GRIDS
    logging::check_fatal(
  ...
  constexpr BoundedPoint() noexcept : x_(XMin - 1), y_(YMin - 1) { }
  constexpr BoundedPoint(class_type&& rhs) noexcept : BoundedPoint(rhs.x(), rhs.y()) { }
  constexpr BoundedPoint(const class_type& rhs) noexcept : BoundedPoint(rhs.x(), rhs.y()) { }

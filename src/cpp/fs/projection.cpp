...
{
  // FIX: moves away from equator in southern hemisphere
  constexpr auto DIST_COMPARE_AT_M = 1'000'000;
  Point p0 = to_lat_long(proj4, x, y0);
  const auto y1 = y0 - DIST_COMPARE_AT_M;
  ...
  from_lat_long(proj4, p1, &x1, &y1);
  // use grid coordinates to determine angle
  const auto angle = abs(Radians{atan2(y1 - y_0, x1 - x)}.asDegrees());
  logging::debug("Angle is {}", angle.value);
  logging::debug(
    "p1 lat/long ({}, {}) becomes grid ({}, {})", p1.latitude(), p1.longitude(), x1, y1
  );

...
class ConstantGrid : public GridData<T, V, const vector<T>>
{
public:
  /**
  ...
  ConstantGrid(
    ...
    : ConstantGrid(
        ...
        yllcorner,
        proj4,
        std::move(vector<T>(static_cast<size_t>(rows) * MAX_COLUMNS, initialization_value))
      )
  {
  }
  /**
  ...
  )
  {
    ...
    for (auto h = 0; h < grid_info.calculateRows(); h += tile_length)
    {
      ...
      const auto y_limit = min(
        static_cast<FullIdx>(tile_length),
      ...
      for (auto w = 0; w < grid_info.calculateColumns(); w += tile_width)
      {
        ...
        const auto x_limit = min(
          static_cast<FullIdx>(tile_width),
          min(grid_info.calculateColumns(), max_column) - w
        );
        for (auto y = y_min; y < y_limit; ++y)
        {
          // read in so that (0, 0) has a hash of 0
          const FullIdx i = static_cast<FullIdx>(MAX_COLUMNS)
                          - (static_cast<FullIdx>(h) + y + offset_y + 1);

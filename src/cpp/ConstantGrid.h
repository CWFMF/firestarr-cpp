...
public:
  ...
  )
  {
    ...
    for (auto h = 0; h < grid_info.rows(); h += tile_length)
    {
      ...
      const auto y_limit = min(tile_length, min(static_cast<int>(grid_info.rows()), max_row) - h);
      for (auto w = 0; w < grid_info.columns(); w += tile_width)
      {
        ...
        const auto x_limit = min(
          tile_width,
          min(static_cast<int>(grid_info.columns()), max_column) - w
        );
    ...
    const auto new_yll = grid_info.yllcorner()
                       + (static_cast<double>(grid_info.rows()) - max_row) * grid_info.cellSize();

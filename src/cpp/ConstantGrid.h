...
public:
  ...
  ) const noexcept override
  {
    ...
    //     logging::check_fatal(location.row() >= this->rows() || location.column() >=
    //     this->columns(), "Out of bounds (%d, %d)", location.row(), location.column());
    // #endif
    // #ifdef DEBUG_POINTS
    // #ifndef DEBUG_GRIDS
    //     const auto location = Location{hash_value};
  ...
  )
  {
    ...
    logging::check_fatal(
      ...
      "Can't have more than %d columns",
      MAX_COLUMNS
    );
    logging::check_fatal(
      max_column > actual_columns,
      "Can't have more than actual %d columns",
      actual_columns
    );
#endif
    auto min_row = max(
...
protected:
  tuple<Idx, Idx, Idx, Idx>
  dataBounds() const override
  {
    Idx min_row = 0;

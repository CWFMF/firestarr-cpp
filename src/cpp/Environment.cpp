...
const BurnedData&
Environment::unburnable() const
{
  return not_burnable_;
}

CellGrid
Environment::makeCells(
  const FuelGrid& fuel,
...
string_view
Environment::proj4() const
{
  return cells_.proj4();
}

Idx
Environment::rows() const
{
  return cells_.rows();
}

Idx
Environment::columns() const
{
  return cells_.columns();
}

MathSize
Environment::cellSize() const
{
  return cells_.cellSize();
}

ElevationSize
Environment::elevation() const
{
  return elevation_;
}

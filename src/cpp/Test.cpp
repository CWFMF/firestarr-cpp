 ...
 */
class TestEnvironment : public topo::Environment
{
public:
  /**
...
public:
  ...
  TestScenario&
  operator=(const TestScenario& rhs) = delete;
  TestScenario&
  operator=(TestScenario&& rhs) = delete;
  /**
   * \brief Constructor
...
)
{
  constexpr auto FMT_FBP_OUT =
    "%8.2f%8.1f%8g%8.1f%8g%8.1f%8.1f%8g%8.1f%8.1f%8.1f%20s%8.3f%8.3f%8c%8ld%8g%8.4g%8.4g%8.4g%s";
  static const vector<const char*> HEADERS{"PREC", "TEMP", "RH",  "WS",  "WD",   "FFMC", "DMC",
                                           "DC",   "ISI",  "BUI", "FWI", "FUEL", "CFB",  "CFC",
                                           "FD",   "HFI",  "RAZ", "ROS", "SFC",  "TFC"};
  printf("Calculated spread is:\n");
  for (auto h : HEADERS)
  {
    printf(0 == strcmp(h, "FUEL") ? "%20s" : "%8s", h);
  }
  printf("\n");
  printf(
    FMT_FBP_OUT,
...
{
  return show_options<V, V>(name, values, "%d", [](V& value) {
    return value;
  });
};
void
...
)
{
  ...
  const auto wind_direction = (fs::wx::Direction::Invalid.asValue()
                               == wx->wind().direction().asValue())
                              ? DEFAULT_WIND_DIRECTION
                              : wx->wind().direction();
  const auto wind_speed = (fs::wx::Speed::Invalid.asValue() == wx->wind().speed().asValue())
                          ? DEFAULT_WIND_SPEED
                          : wx->wind().speed();

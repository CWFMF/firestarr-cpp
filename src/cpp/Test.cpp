...
)
{
  static const map<const char* const, const char* const> FMT{
    {"PREC", "%7.2f"}, {"TEMP", "%7.1f"}, {"RH", "%7g"},    {"WS", "%7.1f"},  {"WD", "%7g"},
    {"FFMC", "%7.1f"}, {"DMC", "%7.1f"},  {"DC", "%7g"},    {"ISI", "%7.1f"}, {"BUI", "%7.1f"},
    {"FWI", "%7.1f"},  {"GS", "%7d"},     {"SAZ", "%7d"},   {"FUEL", "%20s"}, {"GC", "%7g"},
    {"CBH", "%7.1f"},  {"CFB", "%7.3f"},  {"CFC", "%7.3f"}, {"FD", "%7c"},    {"HFI", "%7ld"},
    {"RAZ", "%7d"},    {"ROS", "%7.4g"},  {"SFC", "%7.4g"}, {"TFC", "%7.4g"},
  };
  ...
  for (const auto& h_f : FMT)
  {
    ...
    printf(0 == strcmp(h, "FUEL") ? "%20s" : "%7s", h);
  }
  printf("\n");

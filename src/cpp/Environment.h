...
protected:
  ...
  )
  {
    ...
    for (Idx i = 1; i < elevation.rows() - 1; ++i)
    {
      for (Idx j = 1; j < elevation.columns() - 1; ++j)
      {
        ...
        // Horn's algorithm
        const double dx = ((dem[2] + dem[5] + dem[5] + dem[8]) - (dem[0] + dem[3] + dem[3] + dem[6])
                          )
                        / elevation.cellSize();
        const double dy = ((dem[6] + dem[7] + dem[7] + dem[8]) - (dem[0] + dem[1] + dem[1] + dem[2])
                          )
                        / elevation.cellSize();
        ...
        auto h = loc.hash();
        const auto s_orig = min(static_cast<SlopeSize>(MAX_SLOPE_FOR_DISTANCE), slope.at(h));
    ...
    std::for_each(
      ...
      [&fuel, &slope, &aspect, &values](auto&& h) {
        const auto s = min(static_cast<SlopeSize>(MAX_SLOPE_FOR_DISTANCE), slope.at(h));
  ...
  Environment(
    ...
    : cells_(makeCells(fuel, slope, aspect, elevation))
  {

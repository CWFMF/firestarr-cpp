...
public:
  ...
  findCoordinates(const Point& point, bool flipped) const;
  /**
   ...
   */
  Environment&
  operator=(Environment&& rhs) noexcept = default;
  /**
   ...
   */
  Environment&
  operator=(const Environment& rhs) noexcept = default;
  /**
   ...
   */
  [[nodiscard]] constexpr ElevationSize
  elevation() const
  {
    return elevation_;
  }
  ...
  [[nodiscard]] unique_ptr<data::GridMap<Other>>
  makeMap(
    const Other nodata
  ) const
  {
   ...
   */
  [[nodiscard]] unique_ptr<sim::BurnedData>
  makeBurnedData() const
  {
...
protected:
  ...
  /**
   * \brief Creates a map of areas that are not burnable either because of fuel or the initial
   * perimeter.
   */
  ...
  ) const
  {
    ...
    for (Idx r = 0; r < rows(); ++r)
    {
      for (Idx c = 0; c < columns(); ++c)
      {
        ...
        bool is_outer = 0 == r || 0 == c || (rows() - 1) == r || (columns() - 1) == c;
        // (*result)[location.hash()] = (nullptr ==
        // fuel::fuel_by_code(cells.at(location).fuelCode()));
      ...
      )
  {
    ...
    if (sim::Settings::saveSimulationArea())
    {
      ...
      auto convert_to_fuelcode = [&lookup](const fuel::FuelType* const value) -> FuelSize {
        return lookup.fuelToCode(value);
      };
      if (sim::Settings::saveAsAscii())
      {
        fuel.saveToAsciiFile<FuelSize>(dir_out_, "fuel", convert_to_fuelcode);
        elevation.saveToAsciiFile(dir_out_, "dem");
        // HACK: make a grid with "3" as the value so if we merge max with it it'll cover up
        // anything else
        elevation.saveToAsciiFile<ElevationSize>(dir_out_, "simulation_area", convert_to_area);
      }
      else
      {
        fuel.saveToTiffFile<FuelSize>(dir_out_, "fuel", convert_to_fuelcode);
        elevation.saveToTiffFile(dir_out_, "dem");
        // HACK: make a grid with "3" as the value so if we merge max with it it'll cover up
        // anything else
        elevation.saveToTiffFile<ElevationSize>(dir_out_, "simulation_area", convert_to_area);
      }

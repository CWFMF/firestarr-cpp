    ...
    env_(env)
{
  ...
  const auto nd_for_point = calculate_nd_for_point(env->elevation(), start_point);
...
)
{
  ...
  for (auto& kv : wx)
  {
    ...
    for (auto& kv2 : s)
    {
      auto& day = kv2.first;
      auto& w = kv2.second;
      const fs::wx::Ffmc ffmc(w.tmp(), w.rh(), w.wind().speed(), apcp, last_ffmc);
      const auto t = dates[day];
      const auto month = t.tm_mon + 1;
      const fs::wx::Dmc dmc(w.tmp(), w.rh(), apcp, last_dmc, month, latitude);
      const fs::wx::Dc dc(w.tmp(), apcp, last_dc, month, latitude);
      const fs::wx::Isi isi(w.wind().speed(), ffmc);
      const fs::wx::Bui bui(dmc, dc);
      const fs::wx::Fwi fwi(isi, bui);
      const fs::wx::FwiWeather n(w.tmp(), w.rh(), w.wind(), apcp, ffmc, dmc, dc, isi, bui, fwi);
...
{
  return env_
    ->makeProbabilityMap(for_what, time, start_time, min_value, low_max, med_max, max_value);
}
static void
show_probabilities(
...
)
{
  ...
  vector<double> pct{};
  size_t i = 0;
  auto iterations = readScenarios(start_point, start, save_intensity, start_day, last_date);
...
)
{
  ...
  auto probabilities = model
                         .runIterations(start_point, start, start_day, save_intensity, for_actuals);

...
void
Model::releaseBurnedVector(
  BurnedData* has_burned
) const noexcept
{
    ...
    longitude_(start_point.longitude())
{
  ...
  for (auto day = 0; day < MAX_DAYS; ++day)
  {
    ...
    logging::verbose(
      ...
      fuel::calculate_is_green(nd_.at(static_cast<size_t>(day))) ? "" : " not",
...
)
{
  ...
  auto wx_const = make_shared<ConstantWeather>(
    f,
    start_day - 1,
    weather.dc(),
    weather.dmc(),
    ...
    while (getline(in, str))
    {
      ...
      if (getline(iss, str, ',') && !str.empty())
      {
        ...
        if (wx.find(cur) == wx.end())
        {
          ...
          prev = &yesterday;
          logging::extensive(
            "Resetting new scenario precip to %f from %f",
        ...
        logging::extensive(
          "Adding %f to precip results in accumulation of %f",
          w->prec().asDouble(),
          apcp_24h
        );
        if (12 == t.tm_hour)
...
static void
show_probabilities(
  const map<double, ProbabilityMap*>& probabilities
)
{
...
)
{
  ...
  for (const auto& time : saves)
  {
    result.emplace(
      time,
      model.makeProbabilityMap(time, started, min_value, low_max, med_max, max_value)
    );
  }
  return result;
}
map<double, util::SafeVector*>
make_size_map(
  const vector<double>& saves
)
{
...
)
{
  ...
  auto timer = std::thread([this,
    ...
    logging::debug(
      "Ending timer after %ld seconds with %ld seconds left",
      run_time_seconds,
      time_left
    );
  });
  ...
  auto reset_iter = [&cur_start, this, &mt_extinction, &mt_spread](Iteration& iter) {
    if (Settings::surface())
    {
      ...
      // logging::debug("Applying start location %d/%d (%0.2f)%%", cur_start, n, (100.0 * cur_start)
      // / n);
      auto start_cell = starts_[cur_start];
      // logging::extensive("Applying start #%d (%d, %d)", cur_start, start_cell->row(),
      // start_cell->column()); logging::note("Have %ld save points before reset",
      // iter.getScenarios().at(0)->savePoints().size());
      iter.reset_with_new_start(start_cell);
      // logging::note("Ended up with %ld save points after reset",
      // iter.getScenarios().at(0)->savePoints().size()); logging::extensive("Applied");
      ++cur_start;
    }
    else
    {
...
)
{
  ...
  else
  {
    ...
    {
      logging::fatal(
        "Not enough weather to proceed - have %d days but looking for %d",
        numDays,
        needDays
      );
    }
...
)
{
  ...
  // constexpr auto HEADER_FBP_SECONDARY =
  // "BE,SF,ISI,FFMC,FMC,D0,RSO,CSI,FROS,BROS,HROSt,FROSt,BROSt,FCFB,BCFB,FFI,BFI,FTFC,BTFC,TI,FTI,BTI,LB,LBt,WSV,DH,DB,DF,TROS,TROSt,TCFB,TFI,TTFC,TTI";
  fprintf(out, "%s\r\n", HEADER_FWI);
        ...
        for (SlopeSize slope = 0; slope < SLOPE_MAX; slope += SLOPE_INCREMENT)
        {
          for (AspectSize aspect = 0; aspect < ASPECT_MAX; aspect += ASPECT_INCREMENT)
          {
            ...
            // for (auto fuel_name : FUELS)
            {
              ...
              const fs::sim::SpreadInfo spread(
                year_,
                month,
                day_of_month,
                hour,

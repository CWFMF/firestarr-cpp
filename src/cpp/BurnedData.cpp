...
BurnedData::BurnedData(
  ...
  : data_(from_grid(cells, [](const auto& v) {
      return fuel_by_code(v.fuelCode());
    }))
{
}
...
BurnedData::BurnedData(
  ...
  : data_{from_grid(fuel, [](const auto& v) {
      return v;
    })}
{
}
...
uptr<BurnedData::dtype>
BurnedData::from_grid(
  auto& grid,
  auto fct
)
{
  ...
  for (Idx r = 0; r < grid.rows(); ++r)
  {
    for (Idx c = 0; c < grid.columns(); ++c)
    {
      ...
      (*result)[location.hash()] = is_outer || (nullptr == fct(grid.at(location.hash())));
    }
  }
  return result;
}

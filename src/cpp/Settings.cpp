...
public:
  ...
  ) noexcept
  {
    logging::check_fatal(
      0 > value || 100 < value,
      "Grass curing (%) must be in range [0-100] but got %d",
      value
    );
   ...
   */
  [[nodiscard]] constexpr ThresholdSize
  ...
  {
    output_date_offsets_ = parse_list<int>(value, [](const string s) {
      return stoi(s);
    });
 ...
 */
SettingsImplementation&
SettingsImplementation::instance(
  bool check_loaded
) noexcept
{
...
void
SettingsImplementation::setRoot(
  const char* dirname
) noexcept
{
  try
...
bool
Settings::saveAsTiff() noexcept
{
  return SettingsImplementation::instance().save_as_tiff;
}

void
Settings::setSaveAsTiff(
  const bool value
) noexcept
{
...
bool
Settings::forceNoGreenup() noexcept
{
...
bool
Settings::forceStaticCuring() noexcept
{
  return SettingsImplementation::instance().force_curing;
}

int
Settings::staticCuring() noexcept
{
...
void
Settings::setMaximumTimeSeconds(
  const size_t value
) noexcept
{

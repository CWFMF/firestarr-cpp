...
public:
   ...
   */
  Scenario&
  operator=(Scenario&& rhs) noexcept;
  Scenario&
  operator=(const Scenario& rhs) const = delete;
  /**
   ...
   */
  [[nodiscard]] Scenario*
  reset(mt19937* mt_extinction, mt19937* mt_spread, util::SafeVector* final_sizes);
  /**
   ...
   * \return StartPoint
   */
  [[nodiscard]] constexpr const topo::StartPoint&
  startPoint() const
  {
    return start_point_;
  }
   ...
   */
  [[nodiscard]] constexpr const Model&
  model() const
  {
    return *model_;
  }
   ...
   */
  [[nodiscard]] double
  easting(const topo::Cell& p) const noexcept;
  /**
   * \brief Northing
   * \param p Cell
   * \return Northing
   */
  [[nodiscard]] double
  northing(const topo::Cell& p) const noexcept;
  /**
   ...
   */
  [[nodiscard]] topo::Cell
  cell(const InnerPos& p) const noexcept;
  /**
   ...
   */
  Scenario*
  run(map<double, ProbabilityMap*>* probabilities);
  /**
   * \brief Schedule a fire spread Event
   * \param event Event to schedule
   */
  void
  scheduleFireSpread(const Event& event);
  /**
   ...
   */
  void
  saveIntensity(const string& dir, const string& base_name) const;
  /**

...
struct std::hash<fs::Location>
{
   ...
   */
  [[nodiscard]] constexpr fs::HashSize
  operator()(
...
string
create_file_name(const string_view dir, const string_view base_name, const string_view extension);
...
public:
  ...
  GridBase&
  operator=(const GridBase& rhs) = default;
  GridBase&
  operator=(GridBase&& rhs) noexcept = default;
   ...
   */
  void
  createPrj(const string_view dir, const string_view base_name) const;
  /**
...
GridBase
read_header(TIFF* tif, GTIF* gtif);
GridBase
read_header(const string_view filename);
...
protected:
  ...
  ) const
  {
    ...
    logging::check_fatal(
      (max_column - min_column) % tileHeight != 0,
    ...
    logging::check_fatal(
      0 != (num_columns % tileHeight),
      "%d columns not divisible by tiles",
      num_columns
    );
    ...
    logging::extensive("%s takes %d bits", string(base_name).c_str(), bps);
    ...
    for (size_t co = 0; co < num_columns; co += tileWidth)
    {
      for (size_t ro = 0; ro < num_rows; ro += tileHeight)
      {
        ...
        logging::check_fatal(
          TIFFWriteTile(tif, buf, co, ro, 0, 0) < 0,
          "Cannot write tile to %s",
          filename.c_str()
        );
      }
    }
...
public:
  ...
  {
    return this
      ->template saveToFile<R>(dir, base_name, convert, static_cast<R>(this->nodataInput()));
  }

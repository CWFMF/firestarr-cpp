...
// HACK: this is in C++23 but needs gcc 15
  ...
  {
    insert(end(), rhs.cbegin(), rhs.cend());
  }
#endif
};
 ...
 */
int
sxprintf(char* buffer, size_t N, const char* format, va_list* args);
int
sxprintf(char* buffer, size_t N, const char* format, ...);
 ...
 */
[[nodiscard]] bool
directory_exists(const char* dir) noexcept;
/**
 ...
 */
[[nodiscard]] FileList
read_directory(const string_view name, const string_view match = "*", const bool for_files = true);
/**
 ...
 */
[[nodiscard]] FileList
find_rasters(const string_view dir, YearSize year);
/**
 * \brief Make the given directory if it does not exist
 * \param dir Directory to create
 */
void
make_directory(const char* dir) noexcept;
/**
 * \brief Make the given directory and any parent directories that do not exist
 * \param dir Directory to create
 */
void
make_directory_recursive(const char* dir) noexcept;
 ...
 */
template <class T>
[[nodiscard]] inline constexpr MathSize
sq(
  const T& x
)
{
 ...
 */
tm
to_tm(const YearSize year, const int month, const int day, const int hour, const int minute);
/**
 ...
 */
DurationSize
to_time(const YearSize year, const int month, const int day, const int hour, const int minute);
/**
 ...
 */
void
read_date(istringstream* iss, string* str, tm* t);
...
public:
  ...
  UsageCount&
  operator=(UsageCount&& rhs) noexcept = delete;
  UsageCount&
  operator=(const UsageCount& rhs) noexcept = delete;
  /**
 ...
 */
void
month_and_day(const YearSize year, const size_t day_of_year, size_t* month, size_t* day_of_month);
/**
 ...
 */
[[nodiscard]] bool
is_leap_year(const YearSize year);
/**
 ...
 */
[[nodiscard]] string
make_timestamp(const YearSize year, const DurationSize time);

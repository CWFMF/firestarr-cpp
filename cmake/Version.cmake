...
if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/.git")
  ...
  message("HASH_ENV = ${HASH_ENV}")
  message("FULL_HASH = ${FULL_HASH}")
  if (NOT "${HASH_ENV}" STREQUAL "${FULL_HASH}")
    message("Hashes not equal")
    # add + to version if .env isn't from current commit
    set(VERSION "${VERSION}+")
  else()
      message("Hashes are equal")
  endif()
  # is anything in git changed?
  message("execute_process(COMMAND git diff-index HEAD ${FILES_USED} RESULT_VARIABLE GIT_CHANGED)")
  execute_process(COMMAND git diff-index HEAD ${FILES_USED} RESULT_VARIABLE GIT_CHANGED)
  message("GIT_CHANGED = ${GIT_CHANGED}")
  if ("${GIT_CHANGED}" STREQUAL "0")
    message("Using git time since no change")
